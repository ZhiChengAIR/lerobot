...
#     """整合后的转换函数"""
#     # ==== 步骤1：加载原始数据 ====
#     # 使用原有的load_from_raw函数加载数据（原short版本功能）
#     hdf5_files = sorted(raw_dir.glob("episode_*.hdf5"))
#     num_episodes = len(hdf5_files)
...
#         raw_dir=raw_dir,
#         videos_dir=output_dir / "temp_videos",  # 临时视频目录
#         fps=fps,
#         video=video,
...
#         tasks_by_episodes = df.groupby("episode_index")[tasks_col].first().to_dict()
#     write_jsonlines(tasks, v20_dir / TASKS_PATH)
...
#             "dtype": "float32",
#             "shape": [len(parsed_config["names"]["observation.state"])],
#             "names": parsed_config["names"]["observation.state"]
#         }
#         # 生成动作特征
#         features["action"] = {
#             "dtype": "float32",
#             "shape": [len(parsed_config["names"]["action"])],
#             "names": parsed_config["names"]["action"]
#         }
    
#     # 添加视频特征
#     video_keys = [f"observation.images.{cam}" for cam in get_cameras(raw_dir)]
#     for vid_key in video_keys:
#         video_path = raw_dir / f"episode_0_{cam}.mp4"
#         h, w, c = get_info_from_video(video_path)
#         features[vid_key] = {
#             "dtype": "video",
#             "shape": [h, w, c],
#             "video_info": {
#                 "fps": fps,
#                 "pix_fmt": "yuv420p"  # 根据实际编码调整
#             }
#         }
...
#     # ==== 清理临时文件 ====
#     shutil.rmtree(output_dir / "temp_videos", ignore_errors=True)
