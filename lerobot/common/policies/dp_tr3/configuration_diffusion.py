...
class DiffusionConfig:
    ...
    # This script uses endpose instead of observation.state
    input_shapes: dict[str, list[int]] = field(
        default_factory=lambda: {
            "observation.images": [3, 96, 96],
            "observation.endpose": [7], # xyz rpy gripper
        }
    )
    output_shapes: dict[str, list[int]] = field(
        default_factory=lambda: {
           "action": [7], # xyz rpy gripper
        }
    )
    ...
    # vision_backbone: str = "fpn_resnet34"
    crop_shape: tuple[int, int] | None = (432, 576) # The raw image is (480,640,3), get 90% of the image
    # TODO: Check 480-> 432 or 640->432
    ...
    transformer_n_cond_layers: int = 6
    transformer_n_head: int = 12
    transformer_n_emb: int = 768
    ...
    def __post_init__(self):
        ...
        if self.vision_backbone.startswith("resnet"):
            print("Normal ResNet")
        elif self.vision_backbone.startswith("fpn"):
            print("FPN based ResNet")
        else:
            raise ValueError(
