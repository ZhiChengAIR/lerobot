...
class DiffusionPolicy(nn.Module, PyTorchModelHubMixin):
    ...
    def reset(self):
        ...
        self._queues = {
            ...
            "observation.endpose": deque(maxlen=self.config.n_obs_steps),
...
class DiffusionModel(nn.Module):
    ...
    def __init__(self, config: DiffusionConfig):
        ...
        # Decide if we need a separate encoder per camera or a single encoder
        if self.config.use_separate_rgb_encoder_per_camera and num_images > 1:
            # ----- Option A: multiple encoders, one for each camera -----
            if self.config.vision_backbone == 'resnet18':
                print('*' * 20, flush=True)
                print('vision_backbone: resnet18 (multiple encoders)', flush=True)
                # Create a ModuleList of DiffusionRgbEncoder
                ...
                print('*' * 20, flush=True)
                print('vision_backbone: fpn_resnet34 (multiple encoders)', flush=True)
                # Create a ModuleList of FPNResNetEncoder
                self.rgb_encoder = nn.ModuleList([
                    FPNResNetEncoder('resnet34', pretrained=(config.pretrained_backbone_weights is not None))
            ...
            # ----- Option B: single encoder for all images -----
            if self.config.vision_backbone == 'resnet18':
                print('*' * 20, flush=True)
                print('vision_backbone: resnet18 (single encoder)', flush=True)
                self.rgb_encoder = DiffusionRgbEncoder(config)
            elif self.config.vision_backbone == 'fpn_resnet34':
                print('*' * 20, flush=True)
                print('vision_backbone: fpn_resnet34 (single encoder)', flush=True)
                self.rgb_encoder = FPNResNetEncoder('resnet34', pretrained=(config.pretrained_backbone_weights is not None))
            else:
                raise ValueError(f"Unsupported vision backbone {config.vision_backbone}")

        global_cond_dim = (config.input_shapes["observation.endpose"][0] + self.rgb_encoder.feature_dim * num_images) * config.n_obs_steps
        ...
        #     return num
        def count_parameters(module_or_list: nn.Module) -> float:
            """
            Count the number of trainable parameters (in millions) in a module or a ModuleList of modules.
            """
            if isinstance(module_or_list, nn.ModuleList):
                # Sum parameters across each sub-module
                total_params = 0
                for submodule in module_or_list:
                    total_params += sum(p.numel() for p in submodule.parameters() if p.requires_grad)
                return total_params / 1e6
            else:
                # Single module
                return sum(p.numel() for p in module_or_list.parameters() if p.requires_grad) / 1e6

        # Inside your DiffusionModel class after initialization:
        encoder_params = count_parameters(self.rgb_encoder)
        model_params = count_parameters(self.model)

        if isinstance(self.rgb_encoder, nn.ModuleList):
            print(f"You are using separate camera encoders for each camera ({len(self.rgb_encoder)} total).", flush=True)
        else:
            print("You are using a single shared camera encoder.", flush=True)

        print(f"RGB Encoder parameters: {encoder_params:.1f}M", flush=True)
        print(f"Action Prediction Network parameters: {model_params:.1f}M", flush=True)
    ...
    def _prepare_global_conditioning(self, batch: dict[str, Tensor]) -> Tensor:
        ...
        return torch.cat([batch["observation.endpose"], img_features], dim=-1).flatten(start_dim=1)
